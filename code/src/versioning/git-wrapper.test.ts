...
describe('GitWrapper', () => {
  ...
  beforeEach(() => {
    ...
    testDir = path.join('/tmp', `git-wrapper-test-${Date.now()}-${Math.random().toString(36).substring(7)}`);
    fs.mkdirSync(testDir, { recursive: true });
    
    gitWrapper = new GitWrapper(testDir, {
      userInfo: {
        name: 'Test User',
  ...
  describe('Repository Initialization', () => {
    ...
    it('should set user config when initializing', async () => {
      await gitWrapper.init();
      
      const { stdout: name } = await execAsync('git config user.name', { cwd: testDir });
      const { stdout: email } = await execAsync('git config user.email', { cwd: testDir });
  ...
  describe('File Operations', () => {
    ...
    it('should add files to staging area', async () => {
      ...
      expect(stdout).toContain('A  file1.txt');
      expect(stdout).toContain('A  file2.txt');
    });
    ...
    it('should return empty string when no changes to commit', async () => {
      ...
      const hash = await gitWrapper.addAndCommit(['test.txt'], 'No changes');
  ...
  describe('Status Operations', () => {
    ...
    it('should get status for non-repo directory', async () => {
      const nonRepoWrapper = new GitWrapper('/tmp/non-existent-' + Date.now());
    ...
    it('should detect modified files', async () => {
      fs.writeFileSync(path.join(testDir, 'test.txt'), 'original');
      await gitWrapper.addAndCommit(['test.txt'], 'Add test file');
  ...
  describe('Merge Operations', () => {
    ...
    it('should merge branches', async () => {
      ...
      // Check that merge happened (file from feature branch is now on main)
